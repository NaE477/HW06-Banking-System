...
public class ClerksRep implements UserCRUD<Clerk> {
    ...
    public Clerk readByPresidentId(Integer clientId,Integer presidentId){
        String readQuery = "SELECT * FROM clerks WHERE id = ? AND president_id = ?;";
        try {
            PreparedStatement ps = connection.prepareStatement(readQuery);
            ps.setInt(1,clientId);
            ps.setInt(2,presidentId);
            ResultSet rs = ps.executeQuery();
            if(rs.next()){
                return new Clerk(
                        rs.getString("firstname"),
